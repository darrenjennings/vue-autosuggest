...
export default {
    ...
    return {
      ...
      options: [
        ...
        },
        {
          ...
          name: "blog",
          data: [
            { url: "https://blog.educents.com/", value: "The Best Blog in the Entire World" },
  ...
  methods: {
    onInputChange(text) {
      let filtered = [];
      const suggestionsData = this.options[0].data.filter(item => {return item.toLowerCase().indexOf(text.toLowerCase()) > -1});
      const blogData = this.options[1].data.filter(item => {return item.value.toLowerCase().indexOf(text.toLowerCase()) > -1});
